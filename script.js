...
class EVASystem {
  ...
  descargarArchivoBackup(usuario, llave) {
    const data = {
      usuario: usuario,
      eva_secret_id: llave, // Usamos la misma clave que exige la API
      llave: llave,          // Mantenemos retrocompatibilidad
  ...
  async gestionarArchivoBackup(usuario) {
    return new Promise((resolve) => {
      ...
      if (contenedorBackup) contenedorBackup.style.display = "block";
  ...
  */
    ...
    const infoPeso = document.getElementById('display-peso');
  ...
  finalizarFuerza() {
    ...
    if (lista) {
      lista.innerHTML = `
                <div style="text-align: center; padding: 5px; animation: fadeIn 0.5s ease;">
                    <h2 style="color: #00ff88; font-size: 1.2rem; margin-bottom: 5px;">¡RUTINA COMPLETADA!</h2>
                    <p style="color: #ccc; font-size: 1rem;">Buen trabajo, <span style="color: var(--blue-tech)"> ${this.user} </span>. Has superado todos los ejercicios.</p>
                </div>
            `;
    }
